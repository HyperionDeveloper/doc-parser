...
export class DocsParser {
  ...
  > {
    ...
    const relativeDocsPath = path
      .relative(this.baseVideDir, filePath)
      .split(path.sep)
      .join(path.posix.sep)
      .split('.')[0];
      ...
      if (isTopLevelModuleHeading || isSecondLevelClassHeading) {
        ...
        if (isStructure) {
          ...
          if (isClass) {
            ...
            // FIXME: Make it so that we don't need this magic FIXME for the vide breaking-changes document
