...
if (help) {
  console.info(
    chalk.cyan('Usage: vide-docs-parser --dir ../vide [--out-dir ../vide-out]'),
  );
  process.exit(0);
}
...
if (typeof dir !== 'string') {
  runner.fail(chalk.red('Missing required --dir argument.  "--dir ../vide"'));
  process.exit(1);
}
...
fs.mkdirp(resolvedOutDir).then(() =>
  parseDocs({
    ...
    .then(data =>
      fs.writeJson(path.resolve(resolvedOutDir, './vide-api.json'), data, {
        spaces: 2,
      }),
    )
    .then(() =>
      runner.succeed(
        `${chalk.green('Vide API generated in')} ${chalk.yellow(
          `"${resolvedOutDir}"`,
